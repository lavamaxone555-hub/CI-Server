...
function normalizeEnvironment(value: string | undefined): 'development' | 'test' | 'production' {
  ...
  if (environment !== 'development' && environment !== 'test' && environment !== 'production') throw new Error('NODE_ENV must be development, test, or production')
  return environment
}
...
function loadHealthLatencyLimit(value: string | undefined): number | undefined {
  const raw = value?.normalize('NFC').trim()
  if (raw === undefined || raw === '') return undefined
  const limit = Number(raw)
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 300_000) throw new Error('DATABASE_HEALTH_MAX_LATENCY_MS must be a positive integer')
  return limit
}
...
export function loadPostgresDeploymentConfig(env: Record<string, string | undefined> = process.env): PostgresDeploymentConfig {
  ...
  if (!Number.isSafeInteger(expectedMigrationBaseline) || expectedMigrationBaseline < 1 || expectedMigrationBaseline > 1_000_000) throw new Error('DATABASE_EXPECTED_MIGRATION_BASELINE must be a positive integer')
  ...
  const healthMaxLatencyMs = loadHealthLatencyLimit(env.DATABASE_HEALTH_MAX_LATENCY_MS)
  if (environment === 'production' && healthMaxLatencyMs === undefined) throw new Error('DATABASE_HEALTH_MAX_LATENCY_MS is required in production')

  return { ...database, environment, migrationOnStartup, releaseId, expectedMigrationBaseline, releaseCommitSha, healthMaxLatencyMs }
}
